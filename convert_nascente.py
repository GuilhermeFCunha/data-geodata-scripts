...
for dirpath, dirnames, files in os.walk(src_shapefiles):
    for filename in files:
        if fnmatch.fnmatch(str.lower(filename), '*nascente*'):
...
for shapefile in shapefiles:
    ...
    dst_shapefile = os.path.dirname(shapefile) +"//"+ "aprt_nascente.shp"
    ...
    dst_layer = dst_ds.CreateLayer("limite", dst_sr, ogr.wkbPoint)
    ...
    for src_feature in src_layer:
        ...
        try:
            dst_feature.SetField("tipo", src_feature.GetField("cod_nascente"))
