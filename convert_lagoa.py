...
for dirpath, dirnames, files in os.walk(src_shapefiles):
    for filename in files:
        if fnmatch.fnmatch(str.lower(filename), '*lagoa*'):
...
for shapefile in shapefiles:
    ...
    dst_shapefile = os.path.dirname(shapefile) +"//"+ "aprt_lagoa.shp"
    ...
    field_app = ogr.FieldDefn("app", ogr.OFTInteger)
    field_app.SetWidth(10)
    dst_layer.CreateField(field_app)
